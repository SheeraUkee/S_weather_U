...
sity_abc.add(InlineKeyboardButton(text="Назад", callback_data="back"))

@dp.callback_query_handler(text="АЗБУКА_МОРЗЕ")
async def morze_ABC(message: types.Message):
    await bot.delete_message(message.from_user.id, message.message.message_id)
    await bot.send_message(message.from_user.id, "Выберите город казахстана", reply_markup=sity_abc)
